...
#chatBox {
    background: url("{{ asset('img/Matrix-words.gif') }}") center center / cover no-repeat;
    background-color: rgba(0, 0, 0, 0.85); /* fallback + overlay feel */
    padding: 20px;
...
#chatForm button:hover {
  ...
  box-shadow: inset 0 0 5px #eee, 0 0 8px #00ff88, 0 0 3px silver;
}
</style>
...
@section('scripts')
    <script>
        function selectRace(race) {
            ...
            .then(data => {
                if (data.success) {
                    alert("Race selected: " + data.race_display);
                    location.reload();
                } else {
                    alert(data.message);
                }
...
<meta name="csrf-token" content="{{ csrf_token() }}">
   ...
   <body id="{{ !empty($body_id) ? $body_id : 'ingamepage' }}" 
      class="bg-black text-white font-sans ogame lang-en default no-touch min-h-screen">

    <div class="relative min-h-screen bg-cover bg-center" 
         style="background-image: url('/images/your-background.jpg');">
        <!-- Add any overlay or welcome content here -->
    </div>

    <div id="initial_welcome_dialog" title="Welcome to Dreamforge Emporium!" style="display: none;">
        ...
        start.<br/><br/>
        Remember this is in Alpha-Beta, feedback is much appreciated!
    </div>
    ...
    <div id="middle">
        <div id="eventlistcomponent" class="">
            ...
            <script type="text/javascript">
                ...
                var playerId = "{{ auth()->check() ? auth()->id() : 'null' }};";
                ...
                var session = "3c442273a6de4c8f79549e78f4c3ca50e7ea7580";
                var isMobile = false; //hardcoded for testing
...
<!-- Chat Bar -->
<div id="floatingChatShell" class="collapsed"> <!-- Start in collapsed mode -->
    <div id="chatShellHeader">
        <span class="chat-header-title">Dreamforge Chat</span>
       <div id="toggleChatWrapper">
        <span id="toggleChat" class="chat-minimize-btn">&#43;</span> <!-- Plus = expand -->
    </div>

    <div id="chatShellContent">
        <div class="chat-tabs">
            <div class="chat-tab active">Global</div>
            <div class="chat-tab">Private</div>
        </div>

        <div class="chat-panel active" id="globalChat">
            <ul id="chatMessages">
                <li><strong>System:</strong> Welcome!</li>
            </ul>
        </div>
...
<script type="text/javascript">var visibleChats = {"players": [], "associations": []};
  ...
  border-radius: 4px;
  cursor: pointer;
  box-shadow: inset 0 0 4px #ccc, 0 0 5px #00FFAA;
}
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (chatBar) {
        chatBar.innerHTML = `
            <div id="chatBox">
                <ul id="chatMessages">
                    <li><strong>System:</strong> Welcome to the Dreamforge chat!</li>
                </ul>
                <form id="chatForm">
                    <input type="text" id="chatInput" placeholder="Type your message..." />
                    <button type="submit">Send</button>
...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    ...
    header.addEventListener('click', () => {
      ...
      header.textContent = isVisible ? 'Main Chat ▾' : 'Main Chat ▴';
    });
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  tabs.forEach(tab => {
    tab.addEventListener("click", () => {
      ...
      document.querySelector(`.chat-panel[data-tab="${tab.dataset.tab}"]`).classList.add("active");
    });
  });

  // Default tab
  document.querySelector('.chat-tab[data-tab="global"]').click();
  ...
  chatForm.addEventListener("submit", function (e) {
    ...
    if (msg !== "") {
      const activePanel = document.querySelector('.chat-panel.active ul.chatMessages');
      const li = document.createElement("li");
      li.innerHTML = `<strong>You:</strong> ${msg}`;
      activePanel.appendChild(li);
      chatInput.value = "";
  ...
  const chatContent = document.getElementById("chatShellContent");

  toggleButton.addEventListener("click", function () {
  const chatShell = document.getElementById("floatingChatShell");
  chatShell.classList.toggle("collapsed");

  // Optional: change icon
  toggleButton.innerHTML = chatShell.classList.contains("collapsed") ? "&#43;" : "&#8722;";
});
...
@stack('scripts')
<style>
#floatingChatShell.collapsed #chatShellHeader {
  display: none;
}
#floatingChatShell.collapsed .chat-panel,
#floatingChatShell.collapsed #chatForm {
  display: none;
}
/* When expanded (not collapsed) */
#floatingChatShell:not(.collapsed) #chatShellHeader,
#floatingChatShell:not(.collapsed) .chat-panel,
#floatingChatShell:not(.collapsed) #chatForm {
  display: flex;
}


/* Tabs stay visible in collapsed mode */
#floatingChatShell.collapsed .chat-tabs {
...
/* Default layout (when not collapsed) */
#chatShellHeader,
.chat-panel,
#chatForm {
  display: flex;
...
#floatingChatShell {
  ...
  left: 30px;
  width: 500px;
  background: url("{{ asset('img/Matrix-words.gif') }}") center center / cover no-repeat, #1b1b1b;
  color: #fff;
...
#floatingChatShell.collapsed .chat-minimize-btn {
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        if (toggleButton && chatShell) {
            toggleButton.addEventListener("click", function () {
                chatShell.classList.toggle("collapsed");
                this.innerHTML = chatShell.classList.contains("collapsed") ? "&#43;" : "&#8722;";
            });
        }
    });
</script>
...
<class="bg-black text-green-200">

    <!-- Background Wrapper -->
    <div class="relative min-h-screen bg-cover bg-center" style="background-image: url('/images/your-background.jpg');">

        <!-- Main Content Wrapper - Centers HUD and content -->
    ...
    <div class="w-full max-w-6xl px-4">
            ...
            <!-- HUD Windows -->
            <div class="hud-window hud-top">
                <h2>Top HUD</h2>
                <p>Some important status or stats here...</p>
            </div>

            <div class="hud-window hud-bottom">
